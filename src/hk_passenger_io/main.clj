(ns hk-passenger-io.main
  (:require [reaver :as r :refer [parse extract-from text attr chain]]
...
(defn fetch-all [^LocalDate from-date ^LocalDate to-date]
...
(defn write-csv! [rows fname]
  (with-open [writer (io/writer fname :append true)]
    (csv/write-csv writer
                   rows)))

(defn get-last-record-date [fname]
